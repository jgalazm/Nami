...
!.....DETERMINE THE TOTAL NUMBER OF FAULT SEGMENTS
	  ...
	  DO WHILE (.true.)
		 COUNT = COUNT + 1
		 READ (23,*,end = 991) T1,T2,T3,T4,T5,T6,T7,T8,T9,T10
	  ENDDO
991	continue
	  FLT(1)%NUM_FLT = COUNT
	  REWIND(23)
...
!      INTEGER	   LENGTH, RC, POS !, FLAG
      ...
      OPEN (UNIT=23,FILE=TRIM(LO%DEPTH_NAME),STATUS='OLD',				&
									FORM='FORMATTED')
      IF (ISTAT /=0) THEN
         PRINT *,"ERROR:: CAN'T OPEN WATERDEPTH DATA FILE; EXITING.", ISTAT
         STOP
      END IF
...
!.....DETERMINE THE LENGTH OF BATHYMETRY DATA FILE
	  ...
	  DO WHILE (.true.)
		 COUNT = COUNT + 1
		 READ (23,*,end=992) TEMP1,TEMP2,TEMP3
	  ENDDO
992	  continue
...
!.....DETERMINE THE LENGTH OF BATHYMETRY DATA FILE
	  ...
	  DO WHILE (.true.)
		 COUNT = COUNT + 1
		 READ (23,*,end=993) TEMP1,TEMP2,TEMP3
	  ENDDO
993	  continue
      NXY = COUNT
	  ALLOCATE(XCOL(NXY))
...
!.....DETERMINE THE LENGTH OF ROUGHNESS DATA FILE
	  ...
	  DO WHILE (.true.)
		 COUNT = COUNT + 1
		 READ (23,*,end = 994) TEMP1,TEMP2,TEMP3
	  ENDDO
994	  continue
      NXY = COUNT
	  ALLOCATE(XCOL(NXY))
...
!.....DETERMINE THE LENGTH OF ROUGHNESS DATA FILE
	  ...
	  DO WHILE (.true.)
		 COUNT = COUNT + 1
		 READ (23,*,end=996) TEMP1,TEMP2,TEMP3
	  ENDDO
996	continue
      NXY = COUNT
	  ALLOCATE(XCOL(NXY))
